...
// The shortest trailing slice of a file's logical path that still identifies
// only that file — i.e. the smallest thing you can put in a `{{< ref ... >}}`,
// given that `matchFiles` resolves a reference against the *end* of a logical
// path. For content/blog/coolproject/index.md that is ["coolproject"], unless
// something like content/tags/coolproject/_index.md also exists, in which case
// it grows leftward to ["blog", "coolproject"].
// Returns null when no slice is unique: either two files genuinely share a
// logical path, or the file isn't in the index at all (a new file that has
// never been saved).
/**
 * @param {Entry[]} index
 ...
 * @returns {string[] | null}
 */
function shortestUniqueSlug(index, segments) {
  for (let n = 1; n <= segments.length; n++) {
    const candidate = segments.slice(segments.length - n);
    if (matchFiles(index, candidate).length === 1) return candidate;
  }
  return null;
}
